...
#define LOCKFILE "/home/thematrix/.test/log/gamew.pid"
#define PROGRAMLOCKFILE "/home/thematrix/.test/log/game.pid"
#define LOCKMODE (S_IRUSR | S_IWUSR | S_IROTH | S_IRGRP)

#define STOP_FILE    "/home/thematrix/.test/haha"
#define LOG_FILE     "/home/thematrix/.test/log/w1"
#define LOG_FILE2    "/home/thematrix/.test/log/w2"
...
{  
    ...
    fd = open(PROGRAMLOCKFILE, O_RDWR|O_CREAT, LOCKMODE);
    if( fd < 0 )  {  
        return 1;
    }  
    if( lockfile(fd) < 0 )  
...
void main()
{
    ...
    while(_running)
    {
        ...
        if (check_running() == 0) {
            ...
            char buf[128];
            snprintf(buf, 128, PATH"/%s", daemon_name);
            if (rename(daemon_name, new_name) == 0) {
                mylog(daemon_name);
                strncpy(daemon_name, new_name, 128);
